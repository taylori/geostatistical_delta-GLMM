...
# Chatham rise
# Test default + using a grid (48 sec)
test_that("Chatham Rise hake is working ", {
  #skip_on_travis()
  # Prepping
  test_path = file.path(example_path,"chatham_rise_hake") # Aparrently must run in test_path, and not expample_path
  ...
  Extrapolation_List = SpatialDeltaGLMM::Prepare_Extrapolation_Data_Fn( Region=Region, strata.limits=strata.limits, zone=60 )
